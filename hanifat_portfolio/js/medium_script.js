$(function () {
  var mediumPromise = new Promise(function (resolve) {
    ...
    var data = {
      rss: "https://medium.com/feed/@neefah",
    };
    $.get(
      " https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fmedium.com%2Ffeed%2F%40neefah",
      data,
      function (response) {
        if (response.status == "ok") {
          ...
          $.each(response.items, function (k, item) {
            ...
            display += `<h5 class="card-title"><a target="_blank" href="${item.link}">${item.title}</a></h5>`;
